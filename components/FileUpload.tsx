      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
        ...
        <div className="flex flex-col gap-4 h-full">
            <div className="flex justify-between items-end mb-1">
                <h4 className="text-sm font-semibold text-zinc-900 uppercase tracking-wide">Upload Contracts</h4>
            </div>

            <motion.div 
              whileHover={!disabled ? { scale: 1.02, borderColor: '#52525b' } : {}}
              whileTap={!disabled ? { scale: 0.98 } : {}}
              className={`flex-1 relative group border-2 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center text-center transition-colors duration-300 ${
               disabled 
              ...
              <h3 className="hidden text-lg font-semibold text-zinc-900 mb-2">
                {files.length >= 2 ? 'Limit Reached' : 'Upload Contracts'}
              </h3>
              <p className="text-sm text-zinc-500 max-w-xs leading-relaxed">
                <span className="font-semibold text-zinc-900">Click to browse</span> or drag & drop. 
        ...
        <div className="flex flex-col gap-4 h-full">
            ...
            {files.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center border border-zinc-200 rounded-2xl bg-white text-zinc-400 text-sm p-8 text-center dashed-pattern min-h-[300px]">
